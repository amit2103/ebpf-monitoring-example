...
func (r *requestHandler) HandleBPFEvent(v []byte) {
	...
	case ETSyscallClose:
		if msgInfo, ok := r.FdMap[ev.Attr.Fd]; ok {
			...
			fmt.Println("parser close  ", ev.Attr.EvType)
...
func parseAndPrintMessage(msgInfo *MessageInfo) {
	...
	request, err := http.ReadRequest(bufio.NewReader(&msgInfo.Buf))
	if err != nil {
		fmt.Print("error whuke ", err)
	...
	if printEnabled {
		...
		body.Close()
		fmt.Printf("Len: %s, ContentType: %s, TraceID: %s, Body: %s\n",
			color.GreenString("%d", request.ContentLength),
			color.GreenString("%s", request.Header["Content-Type"]),
			color.GreenString("%s", request.Header["Traceid"]),
...
func main() {
	...
	mustAttachKprobeToSyscall(bccMod, bcc.BPF_PROBE_ENTRY, "read", "syscall__probe_write")
	mustAttachKprobeToSyscall(bccMod, bcc.BPF_PROBE_ENTRY, "close", "syscall__probe_close")

	// Create the output table named "syscall_write_events" that the BPF program writes to.
	table := bcc.NewTable(bccMod.TableId("syscall_write_events_3"), bccMod)
	ch := make(chan []byte)

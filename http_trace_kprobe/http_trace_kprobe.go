...
func (r *requestHandler) HandleBPFEvent(v []byte) {
	...
	fmt.Println("parser calledd  ", ev.Attr.EvType)
...
func main() {
	...
	table := bcc.NewTable(bccMod.TableId("syscall_write_events_2"), bccMod)
	ch := make(chan []byte)

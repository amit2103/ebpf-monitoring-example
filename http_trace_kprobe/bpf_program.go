...
BPF_PERF_OUTPUT(syscall_write_events_2);
...
int syscall__probe_ret_accept4(struct pt_regs *ctx, int sockfd, struct sockaddr *addr, size_t *addrlen, int flags) {
  ...
  syscall_write_events_2.perf_submit(ctx, event, size_to_submit);
...
int syscall__probe_close(struct pt_regs *ctx, int fd) {
  ...
  syscall_write_events_2.perf_submit(ctx, event, sizeof(event->attr));
